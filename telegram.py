...
TOKEN = "2222222222222222222222222222222222222"
OWM_KEY = "2222222222222222222222222222222222222"
...
def parseConfig():
    ...
    URL_OWM = "https://api.openweathermap.org/data/2.5/weather?appid={}&units=metric".format(
        OWM_KEY)
    POLLING_TIMEOUT
    ...
    for update in getResult(updates):
        ...
        try:
            ...
            if (chatId in chats) and (chats[chatId] == "weatherReq"):
                logger.info("Weather requested for %s in chat id %d" %
                            (str(loc), chatId))
        ...
        if text == "/weather":
            ...
            sendMessage(
                "Cahn's Axiom: When all else fails, read the instructions", chatId)
            ...
            sendMessage(
                "I learn new things every day but for now you can ask me about the weather.", chatId, keyboard)
